...
boolean CAcquisitionServer::loop(void)
{
	...
	while(m_vPendingBuffer.size() >= m_ui32SampleCountPerSentBlock*2)
	{
		...
		// push the current chunk of m_vPendingBuffer into the LSL outlet
		std::vector<std::vector<OpenViBE::float32> > chunk(m_vPendingBuffer.begin(),m_vPendingBuffer.begin()+m_ui32SampleCountPerSentBlock);
		m_pLSLOutlet->push_chunk(chunk);
...
boolean CAcquisitionServer::connect(IDriver& rDriver, IHeader& rHeaderCopy, uint32 ui32SamplingCountPerSentBlock, uint32 ui32ConnectionPort)
{
	...
	if(m_pConnectionServer->listen(ui32ConnectionPort))
	{
        ...
        // determine the gender of the subject in string form
        string gender;
        switch (ip_ui64SubjectGender) {
        case 1:
            gender = "Male";
            break;
        case 2:
		...
		// create LSL stream info object
		lsl::stream_info info(rDriver.getName(), "EEG", m_ui32ChannelCount, m_ui32SamplingFrequency, lsl::cf_float32, std::string("OpenViBE_") + rDriver.getName());

		// append whatever meta-data is available (according to specs at code.google.com/p/xdf/)		
		info.desc().append_child("subject")
			.append_child_value("id",boost::lexical_cast<std::string>(ip_ui64SubjectIdentifier).c_str())
			.append_child_value("age",boost::lexical_cast<std::string>(ip_ui64SubjectAge).c_str())
			.append_child_value("gender",gender.c_str());
		
		lsl::xml_element channels = info.desc().append_child("channels");
		for(uint32 i=0; i<m_ui32ChannelCount; i++)
			channels.append_child("channel")
				.append_child_value("label",ip_pSignalMatrix->getDimensionLabel(0,i))
				.append_child_value("type","EEG")
				.append_child_value("unit","microvolts");

		// create stream outlet
		m_pLSLOutlet.reset(new lsl::stream_outlet(info));
