...
{                                                                                                                
	...
	{                                                                                                            
		...
		/// <param name="forward">true for a forward transform, false for                                        
		/// inverse transform</param>                                                                            
		public void RealFFT_ori(double[] data, bool forward)                                                         
		{                                                                                                        
        ...
        public void RealFFT2 ( double[] data, bool forward ) {
            ...
            for (var j = 1; j <= n / 4; ++j) {
                ...
                data[2 * j] = 0.5 * (e + sign * (a + b));
                data[2 * j + 1] = 0.5 * (f + sign * (d - c));
                data[2 * j] = Math.Sqrt (Math.Pow (data[2 * j], 2) + Math.Pow (data[2 * j+1], 2));

                // compute entry y[k]                                                                            
                data[2 * k] = 0.5 * (e - sign * (b + a));
                data[2 * k + 1] = 0.5 * (sign * (d - c) - f);
                data[2 * k] = Math.Sqrt (Math.Pow (data[2 * k], 2) + Math.Pow (data[2 * k + 1], 2));
